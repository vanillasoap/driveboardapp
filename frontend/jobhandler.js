...
jobhandler = {
    ...
    renderBounds: function () {
        ...
        var all_bounds = new paper.Path.Rectangle(new paper.Point(bbox[0] * jobview_mm2px, bbox[1] * jobview_mm2px), new paper.Point(bbox[2] * jobview_mm2px, bbox[3] * jobview_mm2px))
            ...
            for (var poly = 0; poly < path.length; poly++) {
                ...
                if (polyline.length > 1) {
                    ...
                    for (vertex = 1; vertex < polyline.length; vertex++) {
                        ...
                        path_length += Math.sqrt((x - x_prev) * (x - x_prev) + (y - y_prev) * (y - y_prev))
                        jobhandler.bboxExpand(path_bbox, x, y)
                        x_prev = x
                        y_prev = y
                    }
