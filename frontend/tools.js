...
function tools_addfill_init() {
    ...
    var select_html = ''
    // params
    select_html += '<li style="margin-bottom:.5rem">' +
      '<div class="input-group input-group-sm">'+
        '<div class="input-group-prepend">'+
          '<span class="input-group-text" id="pixel">Pixel [mm]</span>'+
        '</div>'+
        '<input id="fillpxsize" type="text" class="form-control pxsize" style="width:2.75rem;" value="'+ app_config_main.pxsize +'" title="size of physical raster pixel in mm">'+
      '</div>' + 
      '</div>' + 
      '</form>' + 
      '</li>'
    ...
    jobhandler.loopItems(function (path, idx) {
        select_html += '<li class="dropdown-item mt-1 mb-1"' + ' id="addfill_' + idx + '" style="background-color:' + path.color + ';"">' + '<a href="#" class="addfill_color" style="color:' + path.color + '">' + '<span class="label label-default kindmem">path</span>' + '<span style="display:none" class="idxmem">' + idx + '</span></a></li>'
    }, "path")
...
function tools_toffset_init() {
    ...
    tools_toffset.offset_set = function (e) {
        ...
        request_get({
            ...
            error: function (data) {
                $().uxmessage('error', "Sorry, Invalid offset position.")
            }
        })
    }
}
...
function tools_tjog_init() {
    ...
    // properties
    group.fillColor = '#000000'
    arrow_up_sm.opacity = 0.4
    arrow_up.opacity = 0.4
    arrow_up_lg.opacity = 0.4
    arrow_down_sm.opacity = 0.4
    arrow_down.opacity = 0.4
    arrow_down_lg.opacity = 0.4
    arrow_left_sm.opacity = 0.4
    arrow_left.opacity = 0.4
    arrow_left_lg.opacity = 0.4
    arrow_right_sm.opacity = 0.4
    arrow_right.opacity = 0.4
    arrow_right_lg.opacity = 0.4
    // create tool
...
function tools_tpos_init() {
    ...
    var yPoint = jobview_height * 0.9875;
